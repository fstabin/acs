...
namespace acs {
	namespace range {
		...
		*/
		...
		class RangeBase {
			RangeBase() {
				static_assert("T is not supported type!!");
			}
		};

		//int型
		template<typename T>
		class RangeBase<T, 0>:public RangeCore<T> {
		public:
			//and/or coudition 
			//isMet
			//時間O(logN)
				...
				if (mIsWhole) {
					...
					case acs::range::LessEqual:
						...
						// x <= 最大でないとき
							...
							// x <= a　は　x < a+1と等価
							mTree.insert({ val + 1, acs::range::Less });//lessを追加
						}
						break;
					case acs::range::Grater:
						mTree.insert({ val, Grater });//graterを追加
					...
					case acs::range::GraterEqual:
						...
						// x >= 最小でない時
							...
							// x >= a　は　x > a-1と等価
							mTree.insert({ val - 1, acs::range::Grater });//gratterを追加
						}
						break;
					case acs::range::Equal:
						mTree.insert({ val, acs::range::Equal });//equalを追加
					...
					case acs::range::LessEqual:
					{
						//andする範囲がすべての時なにも変化しない
						if (val == std::numeric_limits<T>::max())break;
			...
			void orCondition(Condition sign, T val) {
				...
				case acs::range::Equal:
				{
					auto result = mTree.insert({ val, Equal });//equalを追加
					auto treeiter = result.first;
					bool wasSucceeded = false;
					auto beforePair = treeiter;
					auto nextPair = treeiter;
			...
			RangeBase and (const RangeBase& aOther) {
				if (this->mIsWhole && aOther.mIsWhole) {
					Range r;
					r.mIsWhole = true;
					return std::move(r);
		...
		//float型
		template<typename T>
		class RangeBase<T, 1> :public RangeCore<T> {
		public:
			//and/or coudition 
			//isMet
			//時間O(logN)
		...
		template<typename T>using Range = RangeBase < T, static_cast<int>((std::is_integral<T>::value) ? (0) : ((std::is_floating_point<T>::value) ? 1 : (-1))) > ;
	}
}